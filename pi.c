...
void writeDigit(uint8_t range, uint8_t* number) {
    int i;
...
void* timer(void* init) {
    ...
    while(doing) {
        ...
        data = *((uint16_t*)init))(init) - (end - start);
        if(end != now.tv_sec) {
            res = write(fd[1], &data, 2);
...
int UDPserver(char *service) {
    ...
    struct addrinfo *p;
    for(p=origin, resul=origine; p!=NULL; p=p->ai_next)
        if(p->ai_family == AF_INET6) { result = p; break; }
        
    /* create socket */
    int socket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
    if(socket < 0) perror("UDPserver.socket");
        
    /* options  */
    int vrai=1;
    if(setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, &vrai,sizeof(vrai))<0) perror("UDPserver.setsockopt (REUSEADDR)");
        
    /* bind the address */
    res = bind(socket, result->ai_addr, result->ai_addrlen);
...
int onReceived(int socket) {
    while(1){
        struct sockaddr_storage address;
        socklen_t len = sizeof(address);
        //unsigned char data[MAX_UDP];
        uint16_t data;
        int res = recvfrom(s, &data, BUFF-1, 0,(struct sockaddr *) &address, &len);
        ...
        if(resI == 0) {
            printf("Received %d bytes from %s:%s\n (udp) : %d", res, host, service, data);
            char acn[32] = "reçu frère\n";
            if(sizeof(acn) != sendto(socket, acn, sizeof(acn), 0, (struct sockaddr *) &address, len)) perror("onReceived.sendto");
        }
